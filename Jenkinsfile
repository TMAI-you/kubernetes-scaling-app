...
// ID of the Secret File credential holding the ~/.kube/config (MUST match 'kube-config-file')
...
pipeline {
    // The agent must be capable of running Windows commands (bat) and have Docker/Minikube/kubectl installed
    agent any
        ...
        stage('Build Image and Load into Minikube') {
            ...
            steps {
                script {
                    ...
                    // 1. Build the image using the agent's Docker daemon (using bat)
                    bat "docker build -t ${env.FULL_IMAGE_NAME} ."
                    
                    // 2. Load the image into the Minikube cluster's cache (using bat)
                    // This CRITICAL step ensures the cluster can find the image locally
                    bat "minikube image load ${env.FULL_IMAGE_NAME}" 
            ...
            steps {
                script {
                    ...
                    withCredentials([file(credentialsId: KUBE_CONFIG_CREDENTIAL_ID, variable: 'KUBE_CONFIG_PATH')]) {
                        ...
                        // Set the KUBECONFIG environment variable using Windows batch syntax
                        // This makes kubectl use the injected config file.
                        bat "set KUBECONFIG=%KUBE_CONFIG_PATH%"
                        ...
                        if (params.ACTION == 'SCALE_UP') {
                            ...
                            bat "kubectl set image deployment/${K8S_DEPLOYMENT_NAME} ${K8S_DEPLOYMENT_NAME}-container=${env.FULL_IMAGE_NAME} -n ${K8S_NAMESPACE}"
                            
                            // 2. Scale the Deployment
                            bat "kubectl scale deployment/${K8S_DEPLOYMENT_NAME} --replicas=${params.TARGET_REPLICAS} -n ${K8S_NAMESPACE}"
                            
                            // 3. Wait for the rollout
                            bat "kubectl rollout status deployment/${K8S_DEPLOYMENT_NAME} -n ${K8S_NAMESPACE} --timeout=5m" 
